...
public class Hand implements Comparable<Hand> {
    ...
    /**
     * Cheat: Próba zamiany dowolnej karty na asa, o ile taki as jeszcze nie istnieje
     * w ręce gracza, bota, kartach wspólnych ani w talii!
     *
     * @param cardIndex indeks karty w ręce do podmiany
     * @param botHand ręka bota (List<Card>)
     * @param communityCards karty wspólne (List<Card>)
     * @param deckCards pozostałe karty w talii (List<Card>)
     * @return true jeśli udało się podmienić, false jeśli nie (bo nie ma wolnego asa)
     */
    public boolean swapCardForAce(int cardIndex, Card.Suit suit) {
        if (cardIndex < 0 || cardIndex >= cards.size()) {
            throw new IllegalArgumentException("Invalid card index.");
        }
        cards.set(cardIndex, new Card(suit, Card.Rank.ACE));
        evaluate();
        return true;
    }
    ...
    private List<Card> getStraight(List<Card> cardList) {
        List<Integer> unique = cardList.stream()
            .map(c -> c.rank.getValue())
            .distinct()
            .sorted(Comparator.reverseOrder())
            .collect(Collectors.toList());
        if (unique.contains(14)) unique.add(1); // wheel
