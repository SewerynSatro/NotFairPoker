...
public class TableScreen implements Screen {
    ...
    private boolean cheatCaught = false;
    ...
    private String playerHandText = "";
    private String botHandText = "";
    ...
    public void show() {
        if (!initialized) {
            ...
            // Cheats visibility for selected character
    ...
    private void setCheatButtonVisibility() {
        boolean visible = !cheatUsed && gameManager.getGameState() != GameState.SHOWDOWN && !gameManager.isGameOver() && selectedAvatar == 2;
        cheatButton.setVisible(visible);

        visible = !peekUsed && gameManager.getGameState() != GameState.SHOWDOWN && !gameManager.isGameOver() && selectedAvatar == 1;
        peekButton.setVisible(visible);

        // Ukryj oba po showdownie i foldzie lub wykryciu cheata
        if (gameManager.getGameState() == GameState.SHOWDOWN || gameManager.isGameOver()) {
            cheatButton.setVisible(false);
    ...
    private void useCheat(int cardIndex) {
        ...
        Card.Suit chosenSuit = suits[(int) (Math.random() * suits.length)];
        player.hand.swapCardForAce(cardIndex, chosenSuit);
    ...
    private void usePeekCheat() {
        ...
        Card cardToPeek = bot.hand.getCard(0);
    ...
    public void render(float delta) {
        ...
        boolean showBotCards = (gameManager.getGameState() == GameState.SHOWDOWN
            && !gameManager.isShowdownAfterFold()
            && !gameManager.isCheatCaughtThisHand());
        ...
        if (gameManager.getGameState() == GameState.SHOWDOWN) {
            ...
            if (resultText.isEmpty()) {
                resultText = gameManager.getShowdownResultText();
                playerHandText = gameManager.getShowdownPlayerHandText();
                botHandText = gameManager.getShowdownBotHandText();
            }
            font.getData().setScale(2f);
            font.setColor(Color.YELLOW);
            font.draw(batch, resultText, 700, 390);
            font.setColor(Color.WHITE);
            font.getData().setScale(1.3f);
            font.draw(batch, playerHandText, 700, 350);
            font.draw(batch, botHandText, 700, 320);
            font.getData().setScale(2f);
        } else {
            newHandButton.setVisible(false);
        ...
        if (gameManager.isGameOver()) {
            ...
            font.draw(batch, "GAME OVER!", 800, 650);
            font.setColor(Color.WHITE);
            newHandButton.setVisible(false);
            checkButton.setVisible(false);
            callButton.setVisible(false);
